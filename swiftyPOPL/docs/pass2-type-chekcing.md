...
- `MemberResolution`
  - `Field(owner, name)`
  - `Method(owner, name)`
  - `ExistentialMethod(protocol, name)`
...
- `As` 节点表示目前唯一支持的 cast：`e as any P`。
...
- 对 `TypeVariable`：
  ...
  - 多约束同名方法时按完整函数签名（类型参数、值参数/标签、返回类型）判定：
    ...
    - 若同名但签名不同：报错（歧义）。
- 对 `ExistentialProtocol(P)`：
  - 从 `P` 的 requirement 中查找同名方法。
  - 只允许 existential-safe method，即可见参数和返回类型不能泄漏 protocol 中的抽象 `Self`。
  - 解析成功后记录为 `MemberResolution::ExistentialMethod(P, method)`，供 elaborator 生成 core unpack 和 dictionary call。
...
当前 checker 支持 `e as any P`：

1. 先检查 `e` 的类型为 `tau`。
2. 检查 `tau` 是否满足 protocol `P`：
   - concrete struct 通过 `extension S: P` 提供 conformance；
   - type variable 通过泛型约束 `T: P` 提供 evidence。
3. 若满足，则生成 `TypedExpr::As(e, ExistentialProtocol(P), ExistentialProtocol(P))`。

`any P` 不被当作自动满足 `P` 的类型。对 `any P` 的方法调用走 `ExistentialMethod` 路径，而不是普通 conformance 路径。
...
位置：`test/should_pass/` 与 `test/should_fail/`
...
- `existential_show_success.swift`：`e as any P` 打包和 existential method 调用。
- `existential_missing_conformance.swift`：缺少 conformance 时拒绝打包为 `any P`。
- `existential_self_method_rejected.swift`：拒绝 visible signature 中泄漏 `Self` 的 existential method。
- `generic_type_variables.swift`：泛型类型变量、协议约束和 constrained generic 的正向样例。
...
4. 仍是单文件语义检查。
5. 不做隐式泛型实参推导。
6. `any P` 只支持受限 method elimination，不支持 surface-level explicit unpack。
7. existential method 仍要求可见参数和返回类型不泄漏 `Self`。
...
4. 在 Pass3 中将 TypedAST 系统化展开到 IR 或 core term。
5. 为 should-pass / should-fail 目录补统一测试 runner。
