...
Hungry = Rec A. Nat->A;
f = fold [Hungry] (fix (lambda f: Nat->Hungry. lambda n:Nat. fold [Hungry] f));
unfold [Hungry] (unfold [Hungry] (unfold [Hungry] f 0) 1) 2;

Stream = Rec A. Unit->{Nat,A};
hd = lambda s:Stream. (unfold [Stream] s unit).1;
tl = lambda s:Stream. (unfold [Stream] s unit).2;

upfrom0 = fix (lambda f:Nat->Stream. lambda n:Nat. fold [Stream] (lambda _:Unit. {n, f (succ n)})) 0;
hd upfrom0;
hd (tl (tl (tl upfrom0)));

Process = Rec A. Nat->{Nat,A};
p = fix (lambda f: Nat->Process. lambda acc:Nat. fold [Process] (lambda n:Nat.
...
create = fix (lambda f:{x:Nat}->Counter. lambda s:{x:Nat}. fold [Counter]
    {get = s.x,
